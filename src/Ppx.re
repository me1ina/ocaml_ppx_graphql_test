...
let generate_impl = (~ctxt, (_rec_flag, type_declarations), name) => {
  ...
  let generated_data: RecTypes.detectableTypes = GenerateData.generate_data(loc, type_declaration);

  switch (type_declaration) {
    | {ptype_kind: Ptype_abstract, _} => StructureData.create_abstract_structure_item(loc, generated_data, type_name);
    | {ptype_kind: Ptype_record(_), _} => StructureData.create_structure_item_list(loc, generated_data, type_name, name_string);
